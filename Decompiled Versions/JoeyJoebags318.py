...
# Embedded file name: JoeyJoebags3_18.py
# Compiled at: 2018-08-04 03:06:29
...
class Window(Frame):
    ...
    def init_window(self):
        self.master.title('Joey Joebags by BennVenn - V3.18')
        ...
        V5menu = Menu(menu)
        cartTypeMenu.add_cascade(label='Chinese _BV5 Clone Cart NoMod', menu=V5menu)
        V5menu.add_command(label='Erase', command=main_V5_Erase)
        V5menu.add_command(label='Flash ROM', command=main_V5_Write)
        cartTypeMenu.add_separator()
...
def main_ELCheapoSD_Write():
    ...
    USBbuffer = dev.read(129, 64)
    main_ROMBankSwitch(0)
    dev.write(1, [10, 0, 6, 10, 170, 170, 5, 85, 85, 10, 170, 128, 10, 170, 170, 5, 85, 85, 64, 0, 48])
    RAMbuffer = dev.read(129, 64)
    count = 0
    while main_ELCheapo_Read(0)[0] != 255:
        count = count + 1
        if count == 5000:
            print('Failed Erase Blk 0')
            break

    print('Erased0')
    dev.write(1, [10, 0, 6, 10, 170, 170, 5, 85, 85, 10, 170, 128, 10, 170, 170, 5, 85, 85, 96, 0, 48])
    RAMbuffer = dev.read(129, 64)
    while main_ELCheapo_Read(0)[0] != 255:
        count = count + 1
        if count == 5000:
            print('Failed Erase Blk 1')
            break
